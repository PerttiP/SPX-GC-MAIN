...
socket.on("connect", function () {
  ...
  if (document.getElementById("logo") && document.getElementById("logo_off")) {
    document.getElementById("logo").src =
      document.getElementById("logo_on").src;
...
socket.on("SPXMessage2Extension", function (data) {
  ...
  if (typeof window[data.function] === "function") {
    ...
    console.error(
      "Function not found in extension [" + data.function + "]. Is it exposed?"
    );
  }
...
socket.on("SPXMessage2Client", function (data) {
  ...
  switch (data.spxcmd) {
    case "notifyMultipleControllers":
      ...
      if (notification) {
        ...
        if (data.count && data.count > 1) {
          console.log(
            data.count +
              " SPX Controllers detected by the server. Displaying notification."
          );
    ...
    case "updateRundownItem":
      ...
      if (item) {
        let itemTemplate = item.querySelector(
          '[name="RundownItem[relpath]"]'
        ).value;
        if (data.relpath == itemTemplate) {
          ...
          console.warn(
            "Item template mismatch, not updating any indicators in the UI. Request",
            data
          );
        }
      } else {
        console.warn(
          "Requested item " +
            data.itemID +
            " not found. Not updating any indicators in the UI."
        );
      }
      break;
  ...
  switch (data.APIcmd) {
    ...
    case "RundownAllStatesToStopped":
      ...
      items.forEach((item, index) => {
        ...
        item
          .querySelector('[data-spx-name="icon"]')
          .classList.remove("playTrue");
        item
          .querySelector('[data-spx-name="icon"]')
          .classList.remove("playAuto");
    ...
    default:
      console.log(
        "Unknown SPXMessage2Controller command: " + data.APIcmd,
        data
      );
  }
...
socket.on("disconnect", function () {
  ...
  if (document.getElementById("logo") && document.getElementById("logo_off")) {
    document.getElementById("logo").src =
      document.getElementById("logo_off").src;
...
function ajaxpostform(urlPath, FormNro) {} // ajaxpostform ended

function aFunctionTest() {
  // execute a test function on the server
  // this gets triggered by menu > ping
  console.log(
    "Sending testfunction request to server. See server console / log with PING -identifier..."
  );
...
async function AJAXGET(URL) {
  ...
  xmlhttp.onreadystatechange = function () {
    ...
    if (this.readyState == 4 && this.status != 200) {
      console.log(
        "Ajax error, status: " + this.status + ", state: " + this.readyState
      );
      return false;
    }
  };
...
function CancelOutTimerIfRunning(CURITEM) {
  ...
  if (TimerID) {
    ...
    CURITEM.querySelector('[data-spx-name="icon"]').classList.remove(
      "playAuto"
    );

    // Improved in v1.3.1
    let smallIcon = document.getElementById(
      "deleteSmall" + CURITEM.getAttribute("data-spx-index")
    );
    let largeIcon = document.getElementById(
      "deleteLarge" + CURITEM.getAttribute("data-spx-index")
    );
    if (smallIcon) {
      smallIcon.style.display = "inline-block";
    }
    if (largeIcon) {
      largeIcon.style.display = "block";
    }
  }
        ...
        default: // any other key
          if (document.querySelector(".inFocus")) {
            document
              ...
              .setAttribute("data-spx-changed", "true");
          }
          break;
      }
      return;
      ...
      switch (e.keyCode) {
        ...
        case 32: // space
          if (e.shiftKey) {
            ...
            if (
              getFocusedRow().querySelector("#Expanded").style.display != "none"
            ) {
...
function clearUsedChannels(ServerName = "") {
  ...
  data.datafile = document.getElementById("datafile").value;
  working(
    "Clearing ALL graphic channels used by program [" + data.foldername + "]..."
  );
...
async function revealItemID(button) {
  ...
  var newID = prompt(
    "Item ID " + ID + " was copied to clipboard.\nYou can change the ID here:",
    ID
  );
  if (newID != null && newID != ID) {
    ...
    axios
      ...
      .then(function (response) {
        ...
        item
          .querySelector(".copyid")
          .setAttribute("onmouseover", `tip('Copy ID ${newID}')`);
      })
      .catch(function (error) {
        showMessageSlider(
          "Unable to rename. Use unique ID and try again.",
          "error"
        );
      });
...
async function revealItemTiming(button) {
  ...
  var newValue = prompt(
    "You can change timing here. Use 'manual', 'none' or millisecond values (1000=1s):",
    curValue
  );
  if (newValue != null && newValue != curValue) {
    ...
    axios
      ...
      .then(function (response) {
        ...
        button.closest(".itemrow").querySelectorAll("input[id^=out]")[0].value =
          newValue;
      })
      .catch(function (error) {
...
async function revealItemLayer(button) {
  ...
  let ID = button.closest(".itemrow").getAttribute("data-spx-epoch");
  var newValue = prompt(
    "You can change web playout layer here. Use any value between 1 and 20:",
    curValue
  );
  if (newValue != null && newValue != curValue) {
    let file = document.getElementById("datafile").value;
    ...
    axios
      ...
      .then(function (response) {
        ...
        button
          .closest(".itemrow")
          .querySelectorAll("input[id^=webplayout]")[0].value = newValue;
...
function heartbeat(dd, getOnly = false) {
  // 36 24 36 hey
  ...
  ar.every(function (f, idx) {
    ...
    if (key == eK) {
      ...
      if (Math.round((Date.now() - val) / (1000 * 60 * 60 * 24)) > 0) {
        rp = true;
      }
      // ###################
      // rp = true; // debug
...
function del() {
  ...
  if (confirm('SURE?! Delete "' + filename + "\" and all it's data?")) {
    ...
    setTimeout(function () {
      document.location = "/show/" + foldname;
    }, 500);
  }
} // del ended
...
function duplicateRundown() {
  ...
  setTimeout(function () {
    document.location = "/show/" + data.foldname;
  }, 500);
...
function duplicateRundownItem(rowitem) {
  ...
  let list = [
    ...
    newItem
      ...
      .setAttribute(
        "onmouseover",
        'tip("Duplicated item ' + data.cloneEpoch + '")'
      ),
    (newItem.querySelector(".utilityOverlay").innerHTML =
      "Reload page to update this duplicated item."),
    newItem.querySelector('[data-spx-name="icon"]').classList.add("playFalse"),
    newItem
      .querySelector('[data-spx-name="playbutton"]')
      .classList.add("bg_green"),
    newItem
      .querySelector('[data-spx-name="icon"]')
      .classList.remove("playTrue"),
    newItem
      .querySelector('[data-spx-name="playbutton"]')
      .classList.remove("bg_red"),
    (newItem.querySelector('[data-spx-name="playbutton"]').textContent = newItem
      .querySelector('[data-spx-name="playbutton"]')
      .getAttribute("data-spx-playtext")),
...
function openLightModePanel() {
  ...
  window.open(
    newUrl,
    "SPX-LITE",
    "_blank, width=680, height=850, scrollbars=yes, location=yes,status=yes"
  );
}
...
function ModalOn(modalID) {
  ...
  if (document.getElementById("allToggle")) {
    document.getElementById("allToggle").innerText = document
      .getElementById("allToggle")
      .getAttribute("data-all-label");
  }
...
function filterProjects() {
  ...
  for (i = 0; i < optionItems.length; i++) {
    ...
    filters.forEach((filterItem) => {
      shouldDisplay =
        shouldDisplay && txtValue.toUpperCase().includes(filterItem);
    });
...
function getElementIdOfFocusedItem() {
  ...
  for (let nro = 0; nro < document.querySelectorAll(".itemrow").length; nro++) {
    if (
      document.querySelectorAll(".itemrow")[nro].classList.contains("inFocus")
    ) {
...
function getElementByEpoch(itemID) {
  ...
  for (
    let index = 0;
    index < document.querySelectorAll(".itemrow").length;
    index++
  ) {
    DomItemID = document
      .querySelectorAll(".itemrow")
      [index].getAttribute("data-spx-epoch");
...
function getDomIndexByElementId(ElementId) {
  ...
  for (
    let DomNro = 0;
    DomNro < document.querySelectorAll(".itemrow").length;
    DomNro++
  ) {
    if (
      document.querySelectorAll(".itemrow")[DomNro].id ==
      "playlistitem" + String(ElementId)
    ) {
...
function getMessages(curVerInfo) {
  ...
  fetch(url)
    ...
    .then((messages) => {
      ...
      if (
        (messages.latest.vers && versInt(latestVer) > versInt(currntVer)) ||
        debugNotifications
      ) {
        ...
        document.getElementById("upgrade_date").innerText =
          messages.latest.date;
        document.getElementById("upgrade_head").innerText =
          messages.latest.head;
        document.getElementById("upgrade_body").innerText =
          messages.latest.body;
        document.getElementById("upgrade_call").innerText =
          messages.latest.call;
        document.getElementById("upgrade_link").href = messages.latest.href;
        document.getElementById("upgrade_link").innerText =
          messages.latest.link;
      }

      if (
        (messages.notification.type && messages.notification.type != "none") ||
        debugNotifications
      ) {
        // Yes, there is something else to announce
        document.getElementById("messageinfo").style.display = "block";
        document.getElementById("messageinfo").classList =
          "message-" + messages.notification.type;
        document.getElementById("messagelink").classList =
          "message-" + messages.notification.type;
        document.getElementById("message_date").innerText =
          messages.notification.date;
        document.getElementById("message_head").innerText =
          messages.notification.head;
        document.getElementById("message_body").innerText =
          messages.notification.body;
        document.getElementById("message_call").innerText =
          messages.notification.call;
        document.getElementById("message_link").href =
          messages.notification.href;
        document.getElementById("message_link").innerText =
      ...
      if (messages.homepagepromo[versionStr]) {
        ...
        document.getElementById("homepagepromolink").href =
          messages.homepagepromo[versionStr].link;
        document.getElementById("homepagepromopict").src =
          messages.homepagepromo[versionStr].pict;
        document.getElementById("homepagepromopict").title =
          messages.homepagepromo[versionStr].titl;
      } else {
        // Check if there is a default promo
        if (
          messages.homepagepromo &&
          messages.homepagepromo.default &&
          messages.homepagepromo.default.active
        ) {
          // console.log('Using active default promo.', messages.homepagepromo.default);
          document.getElementById("homepagepromolink").href =
            messages.homepagepromo.default.link;
          document.getElementById("homepagepromopict").src =
            messages.homepagepromo.default.pict;
          document.getElementById("homepagepromopict").title =
            messages.homepagepromo.default.titl;
        }
      }
    })
    .catch((error) => {
...
function handleRendererPopups(data) {
  ...
  var OPT =
    "_blank,toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=" +
    w +
    ", height=" +
    h +
    ", top=" +
  ...
  if (data.type === "preview") {
    URL = "/renderwindow/preview";
    winRef = "SPXpreviewWindow";
  }
  if (data.type === "program") {
    URL = "/renderwindow/program";
    winRef = "SPXprogramWindow";
  }
...
function help(section) {
  ...
  switch (section) {
    case "INTRO":
      HELP_PAGE = "article/help-intro";
      break;
    case "PROJECTS":
      HELP_PAGE = "article/help-dataroot";
      break;
    case "PLAYLISTS":
      HELP_PAGE = "article/help-dataroot";
      break;
    case "CONFIG":
      HELP_PAGE = "article/help-config-general";
      break;
    case "WEBPLAYOUT":
      HELP_PAGE = "article/help-config-renderer";
      break;
    case "CASPARCG":
      HELP_PAGE = "article/help-config-casparcg";
      break;
    case "GLOBALEXTRAS":
      HELP_PAGE = "article/help-config-globalextras";
      break;
    case "PROJECTGENERAL":
      HELP_PAGE = "article/help-project-general";
      break;
    case "PROJECTTEMPLATES":
      HELP_PAGE = "article/help-project-templates";
      break;
    case "PROJECTVARIABLES":
      HELP_PAGE = "article/help-project-variables";
      break;
    case "PROJECTEXTRAS":
      HELP_PAGE = "article/help-project-extras";
      break;
    case "CONTROLLER":
      HELP_PAGE = "article/help-controller";
      break;
    case "CSV":
      HELP_PAGE = "article/help-csv-files";
      break;
    case "API":
      HELP_PAGE = "article/help-api";
      break;
    case "ITEM-DETAILS":
      HELP_PAGE = "article/help-item-details";
      break;
    case "SEVERAL":
      HELP_PAGE = "article/help-several-controllers";
      break;
    case "LIGHTMODE":
...
function moveFocus(nro) {
  ...
  for (let item = 0; item < itemCount; item++) {
    if (
      document.querySelectorAll(".itemrow")[item].classList.contains("inFocus")
    ) {
      curIndex = item;
    }
  }
...
function nextItem(itemrow = "") {
  ...
  if (stepsleft < 1) {
    ...
    itemrow.setAttribute(
      "data-spx-stepsleft",
      itemrow.querySelector('input[name="RundownItem[steps]"]').value
    ); // reset counter
    let uselessReturnValue = setItemButtonStates(
      itemrow,
      (forcedCommand = "stop")
    );
  ...
  let onNextField = itemrow.querySelector(
    '[name="RundownItem[function_onNext]"]'
  );
  ...
  if (onNextField && onNextField.value != "") {
    ...
    let eventCondtFF = itemrow.querySelector(
      "[data-update=" + ConditFField + "]"
    );
    if (eventCondtFF) {
...
function previewItem(itemrow = "") {
  ...
  if (!itemrow) {
    /* console.log('No active rows, skip command.');*/ return;
  }
...
function playItem(itemrow = "", forcedCommand = "", preventPlayout = false) {
  ...
  let onPlayField = itemrow.querySelector(
    '[name="RundownItem[function_onPlay]"]'
  );
    ...
    setTimeout(function () {
      if (window[FunctionName]) {
        ...
        showMessageSlider(
          'Function "' +
            FunctionName +
            '" not found in current custom functions library file.',
          "warn"
        );
      }
    }, parseInt(ExecuteDelay));
  ...
  let onStopField = itemrow.querySelector(
    '[name="RundownItem[function_onStop]"]'
  );
  ...
  let TimeoutAsString = itemrow.querySelector(
    '[name="RundownItem[out]"]'
  ).value;
...
function setItemButtonStates(itemrow, forcedCommand = "") {
  ...
  if (
    itemrow.getAttribute("data-spx-onair") == "true" ||
    forcedCommand == "stop"
  ) {
  ...
  if (CommandToExecute == "play") {
    ...
    let PlayoutConfig = itemrow
      .querySelector('[data-spx-name="playoutConfig"]')
      .innerText.split(" ")
      .join("-");
    rows.forEach(function (item, index) {
      let CurrentConfig = item
        .querySelector('[data-spx-name="playoutConfig"]')
        .innerText.split(" ")
        .join("-");
      if (
        item.getAttribute("data-spx-onair") == "true" &&
        CurrentConfig == PlayoutConfig
      ) {
        ...
        curExpandedPlay.innerText =
          curExpandedPlay.getAttribute("data-spx-playtext");
  ...
  if (ife("identifier").value == "controller") {
    ...
    rows.forEach(function (item, index) {
      if (item.getAttribute("data-spx-onair") == "true") {
        ...
        item.querySelector('[data-spx-name="deletesmall"]').style.visibility =
          "hidden" || console.log("a");
        item.querySelector('[data-spx-name="deletelarge"]').style.visibility =
          "hidden" || console.log("b");
      } else {
        // console.log('Show delete buttons of ' + item.id);
        item.querySelector('[data-spx-name="deletesmall"]').style.visibility =
          "visible" || console.log("c");
        item.querySelector('[data-spx-name="deletelarge"]').style.visibility =
          "visible" || console.log("d");
      }
    });
  }
...
function SaveNewSortOrder() {
  console.debug("PERTTI DEBUG: saveNewSortOrder in spx_gc!");
...
function saveTemplateItemChanges(elementID) {
  console.debug("PERTTI DEBUG: saveTemplateItemChanges in spx_gc!");
  ...
  [...form.elements].forEach((input, index) => {
    if (input.getAttribute("data-role") == "userEditable") {
      ...
      if (
        document.getElementById("datapreview_" + elementID + "_" + iterator)
      ) {
        ...
        document.getElementById(
          "datapreview_" + elementID + "_" + iterator
        ).innerText = temp2;
      }
    }
  });
...
function saveTemplateItemChangesByElement(itemrow) {
  ...
  if (typeof runSaveDataFromTemplateUpdate() === "function") {
    ...
    console.error(
      "runSaveDataFromTemplateUpdate() function missing from SPX template."
    );
  }
*/
  /*
  if (typeof window.runSaveDataFromTemplateUpdate() === "function") {
    window.runSaveDataFromTemplateUpdate(); // Update will follow if User presses PLAY next
  } else {
    console.error(
      "window.runSaveDataFromTemplateUpdate() function missing from SPX template."
    );
  }
*/

  // Then "send" a message (this message will be emitted to the server,
  // which you can also handle to perform client-side navigation if needed)
  /*
  socket.emit("template_rundown_item_saved", {
    message: "Template rundown item changes saved",
    timestamp: Date.now(),
  });
*/
...
function setMasterButtonStates(itemrow, debugMessage = "") {
  ...
  if (
    itemrow.querySelector('input[name="RundownItem[out]"]') &&
    itemrow.querySelector('input[name="RundownItem[steps]"]').value == "none"
  ) {
  ...
  var stepsVal =
    itemrow.querySelector('input[name="RundownItem[steps]"]').value || 0;
  ...
  if (stepsleft > 1 && onair) {
    setTimeout(function () {
      CONTINBUTTON.classList.remove("disabled");
    }, 3);
  }
  ...
  switch (sysCmd) {
    case "OPENWEBRENDERER":
      window.open(
        "/renderer",
        "gcwebrender",
        "width=1920,height=1080,scrollbars=yes,location=yes,status=yes"
      );
      break;
...
function spxInit() {
  ...
  // - init Sortable
  console.log(
    "%c  SPX Graphics Controller (c) 2020- SPX Graphics  ",
    "border-radius: 200px; font-size: 1.1em; padding: 0.4em; background: #0e7a27; color: #fff"
  );
...
function toggleLRendererHandler(slider, targ) {
  ...
  if (slider.checked) {
    ...
    document.getElementById("previewIF").src = "/templates/empty.html";
    document.getElementById(
      "LocalRendererDisabledNotification"
    ).style.opacity = 1;
    Cfgdata.val = true;
  } else {
    document.getElementById("previewIF").src = "/renderer";
...
function updateItem(itemrow) {
  console.debug("PERTTI DEBUG: updateItem in spx_gc!");
  ...
  data.playserver = itemrow.querySelector(
    '[name="RundownItem[playserver]"]'
  ).value;
  data.playchannel = itemrow.querySelector(
    '[name="RundownItem[playchannel]"]'
  ).value;
  data.playlayer = itemrow.querySelector(
    '[name="RundownItem[playlayer]"]'
  ).value;
  data.webplayout = itemrow.querySelector(
    '[name="RundownItem[webplayout]"]'
  ).value;
  data.epoch = itemrow.getAttribute("data-spx-epoch");
...
function playServerAudio(sfx, message = "") {
  console.log(
    "%cPlaying sound (" + sfx + ") on server. Log: " + message + ".",
    "background: #622; color: #fff"
  );
  data = {};
  data.sound = sfx;
  data.info = message;
